...
import {
  ...
  TextField, Button, CircularProgress, Switch,
} from '@mui/material';
...
import { linkExistingCustomer, createCustomerAndLink } from '../store/customersSlice';
import { linkExistingLocation, createLocationAndLink } from '../store/locationsSlice';
...
const getCol = (item, colId) => item?.column_values?.find((cv) => cv.id === colId);
...
const getLinkedId = (item, colId) => {
  ...
  try {
    ...
    if (ids.length > 0) {
      const id = typeof ids[0] === 'object' ? ids[0].linkedPulseId || ids[0].id : ids[0];
      return String(id);
    }
  } catch { /* ignore */ }
  return '';
...
const getMultiDay = (item) => {
  ...
  try { return col?.value ? JSON.parse(col.value)?.checked === true || JSON.parse(col.value)?.checked === 'true' : false; }
  catch { return false; }
};
...
const Section = ({ children }) => (
  <Typography sx={{
    ...
    textTransform: 'uppercase', color: '#b0ada8', px: 1, mb: 0.25, mt: 0.25,
  }}>
    {children}
  </Typography>
);
...
const PropertyRow = ({ icon: Icon, label, children }) => (
  <Box sx={{
    display: 'grid', gridTemplateColumns: '160px 1fr', alignItems: 'start',
    borderRadius: '4px', px: 1, py: '7px',
...
const InlineField = ({ value, onChange, placeholder, multiline, rows }) => (
  <TextField
    ...
    sx={{
      ...
      '& .MuiInputBase-input': { p: 0, lineHeight: 1.55, '&::placeholder': { color: '#c1bfbc', opacity: 1 } },
    ...
    {options.map((opt) => {
      ...
      return (
        <Chip key={opt} label={opt} size="small"
          ...
          sx={{
            fontSize: '0.7rem', height: 20, fontWeight: 600, cursor: 'pointer',
            bgcolor: active ? color : 'transparent', color: active ? '#fff' : '#6b7280',
            border: '1px solid', borderColor: active ? color : '#e5e7eb',
            '&:hover': { bgcolor: active ? color : '#f3f4f6' }, transition: 'all 0.1s',
          }} />
      );
    })}
  </Stack>
...
export default function WorkOrderDetailDrawer({ open, onClose, workOrder }) {
  ...
  const [form, setForm] = useState(() => ({
    name:            workOrder?.name || '',
    description:     getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.DESCRIPTION),
    status:          getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.STATUS),
    scheduledDate:   getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.SCHEDULED_DATE),
    multiDay:        getMultiDay(workOrder),
    serviceHistory:  getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.SERVICE_HISTORY),
    workPerformed:   getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.WORK_PERFORMED),
    executionStatus: getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.EXECUTION_STATUS),
    partsOrdered:    getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.PARTS_ORDERED),
  ...
  const woId           = workOrder ? getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.WORKORDER_ID) : '';
  const model          = workOrder ? getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.MODEL) : '';
  const serialNumber   = workOrder ? getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.SERIAL_NUMBER) : '';
  const equipment      = workOrder ? getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.EQUIPMENT) : '';
  const technician     = workOrder ? getColText(workOrder, MONDAY_COLUMN_IDS.WORK_ORDERS.TECHNICIAN) : '';

  const handleSave = async () => {
    await dispatch(updateWorkOrder({ workOrderId: workOrder.id, form })).unwrap();
    onClose();
  };
  ...
  return (
    <>
      <Drawer
        ...
        PaperProps={{
          sx: {
            width: 520, bgcolor: '#fff', display: 'flex', flexDirection: 'column',
      ...
      >
        ...
        <Box sx={{ px: 3, pt: 3, pb: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <Box sx={{ flex: 1, mr: 2 }}>
              {woId && (
                <Typography sx={{ fontSize: '0.72rem', fontWeight: 600, color: '#9b9a97', letterSpacing: '0.04em', mb: 0.4, fontFamily: 'monospace' }}>
                  WO # {woId}
                </Typography>
              )}
              {/* Editable title */}
              <TextField
                ...
                sx={{
                  '& .MuiInput-root': { fontSize: '1.1rem', fontWeight: 700, color: '#37352f', '&:before, &:after': { display: 'none' } },
                  '& .MuiInputBase-input::placeholder': { color: '#c1bfbc', opacity: 1 },
                }}
              />
              <Typography sx={{ fontSize: '0.78rem', color: '#9b9a97', mt: 0.3 }}>Work order details</Typography>
            </Box>
            <IconButton size="small" onClick={onClose}
              sx={{ borderRadius: '5px', color: '#9b9a97', '&:hover': { bgcolor: '#f1f1ef', color: '#37352f' }, flexShrink: 0 }}>
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', px: 2.5, py: 2.5 }}>
          ...
          <Box sx={{ mb: 2.5 }}>
            <PropertyRow icon={PersonOutlineIcon} label="Customer">
              <RelationCell
                ...
                onSelectExisting={(id, name) => {
                  set('customerId', id); set('customerName', name);
                  dispatch(linkExistingCustomer({ workOrderId: workOrder.id, customerId: id, customerName: name, previousSnapshot: { value: null, text: null, display_value: null } }));
                }}
            ...
            <PropertyRow icon={LocationOnOutlinedIcon} label="Location">
              <RelationCell
                ...
                onSelectExisting={(id, name) => {
                  set('locationId', id); set('locationName', name);
                  dispatch(linkExistingLocation({ workOrderId: workOrder.id, locationId: id, locationName: name, previousSnapshot: { value: null, text: null, display_value: null } }));
                }}
                onCreateNew={(v) => setPendingNewLocation({ name: v })}
              />
            </PropertyRow>
            <PropertyRow icon={VerifiedOutlinedIcon} label="Status">
              <StatusChips options={STATUS_OPTIONS} hexMap={STATUS_HEX} value={form.status} onChange={(v) => set('status', v)} />
            </PropertyRow>
            <PropertyRow icon={EngineeringOutlinedIcon} label="Technician">
              <Typography sx={{ fontSize: '0.875rem', color: technician ? '#37352f' : '#c1bfbc' }}>
                {technician || '—'}
              </Typography>
            </PropertyRow>
          ...
          {/* Equipment (read-only mirror columns) */}
              ...
              <Box sx={{ mb: 2.5 }}>
                  ...
                  <PropertyRow icon={BuildOutlinedIcon} label="Equipment">
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {equipment.split(',').map((e, i) => (
                        <Chip key={i} label={e.trim()} size="small" sx={{ height: 20, fontSize: '0.73rem', bgcolor: 'rgba(34,197,94,0.1)', color: '#15803d', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '4px', '& .MuiChip-label': { px: 1 } }} />
                      ))}
                    </Box>
                  </PropertyRow>
                )}
                {model && (
                  <PropertyRow icon={TagIcon} label="Model">
                    <Typography sx={{ fontSize: '0.875rem', color: '#37352f' }}>{model}</Typography>
                  </PropertyRow>
                )}
                {serialNumber && (
                  <PropertyRow icon={QrCodeOutlinedIcon} label="Serial Number">
                    <Typography sx={{ fontSize: '0.875rem', color: '#37352f', fontFamily: 'monospace' }}>{serialNumber}</Typography>
                  </PropertyRow>
                )}
              </Box>
            </>
          ...
          <Box sx={{ mb: 2.5 }}>
            <PropertyRow icon={VerifiedOutlinedIcon} label="Execution Status">
              <StatusChips options={EXECUTION_STATUS_OPTIONS} hexMap={STATUS_HEX} value={form.executionStatus} onChange={(v) => set('executionStatus', v)} />
            </PropertyRow>
            <PropertyRow icon={InventoryOutlinedIcon} label="Parts Ordered">
              <StatusChips options={PARTS_ORDERED_OPTIONS} hexMap={PARTS_HEX} value={form.partsOrdered} onChange={(v) => set('partsOrdered', v)} />
            </PropertyRow>
          </Box>
        ...
        <Box sx={{ px: 3, py: 2, borderTop: '1px solid #e8e6e1', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button onClick={onClose} disabled={saving}
              sx={{ px: 2, height: 32, borderRadius: '4px', fontSize: '0.82rem', fontWeight: 500, color: '#787774', textTransform: 'none', '&:hover': { bgcolor: '#f1f1ef' } }}>
              Cancel
            </Button>
            <Button onClick={handleSave} variant="contained" disableElevation disabled={saving}
      ...
      {/* Nested drawers for creating new customer/location */}
      <CustomerDrawer
        open={!!pendingNewCustomer}
        customer={pendingNewCustomer ? { id: '__new__', name: pendingNewCustomer.name, column_values: [] } : null}
        onClose={() => setPendingNewCustomer(null)}
        onSaveNew={async (custForm) => {
          await dispatch(createCustomerAndLink({ form: custForm, workOrderId: workOrder.id }));
          setPendingNewCustomer(null);
        }}
      />
      <LocationDrawer
        open={!!pendingNewLocation}
        location={pendingNewLocation ? { id: '__new__', name: pendingNewLocation.name, column_values: [] } : null}
        onClose={() => setPendingNewLocation(null)}
        onSaveNew={async (locForm) => {
          await dispatch(createLocationAndLink({ form: locForm, workOrderId: workOrder.id }));
